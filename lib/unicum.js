...
//         console.log(JSON.stringify(this._keyTypeIndexes));
    ...
    generate: function (ktype, quantity, cb) {
        var thiz = this;
        thiz._getClient().time(function (err, val) {

            var sec = val[0] - thiz._epoch,
                microsec = val[1];
            ...
            if (quantity == 1) {
                var k = sec62 + microsec62 + ktype62;
                callback(k);
            } else {
                var ret = [],
                	v = val[1];
                for (var j = 0; j < quantity; j++) {
                    ret[j] = sec62 + thiz.toInt62(v, 4) + ktype62;
                    v++;
                    if (v == 1000000) {
                        v = 0;
    ...
    customKey: function (sec, microsec, ktype, suffix) {
    	var key = (sec ? this.toInt62(sec) : "") 
        	+ (sec || microsec ? this.toInt62(microsec, 4) : "") 
    ...
    _arrange: function (k) {
        ...
        if (this._keyTypeIndexes[ktype]) {
            var sec32 = key.substring(0, l - 4),
	        	microsec32 = key.substring(l - 4, l - 2);
            return {
    ...
    getTime: function (key) {
        ...
        if (typeof key == 'string') {
        	key = key.split("$")[0] || "0";
            var l = key.length;
            var sec = this.fromInt62(key.substring(0, l - 6));
        }
        return sec + this._epoch;
    },
